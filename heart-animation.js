...
// Глобальные переменные для интерактивности мыши
...
document.addEventListener('click', function() {
  extraScale = 0.2; // Дополнительный импульс при клике
});
...
let init = function () {
  ...
  let loop = function () {
    let n = -Math.cos(time);
    pulse((1 + n) * 0.5 + extraScale, (1 + n) * 0.5 + extraScale);
    // Постепенное затухание эффекта клика
    extraScale *= 0.98;
  ...
  // Анимация пульсации текста валентинки
  function animateGreeting() {
    var greeting = document.getElementById('greeting');
    if (greeting) {
      var t = Date.now() * 0.002;
      var opacity = 0.7 + 0.3 * Math.sin(t);
      greeting.style.opacity = opacity;
    }
    window.requestAnimationFrame(animateGreeting, canvas);
  }
  animateGreeting();
};
